...
typedef struct {
	V3 pos;
	//This is to keep the position seperate from the shaking
	V3 hidden_pos;
...
//NOTE: Camera Shaking
typedef enum {
	EASY_CAMERA_SHAKE_DEFAULT
} EasyCamera_ShakeType;

static inline void easyCamera_startShake(EasyCamera *cam, EasyCamera_ShakeType type, float magnitude) {
...
static inline void easy3d_updateCamera(EasyCamera *cam, AppKeyStates *keyStates, float sensitivity, float movePower, float dt, EasyCamera_MoveType moveType) {
	...
	cam->hidden_pos = v3_plus(v3_scale(dt, cam->velocity), cam->hidden_pos);

	// printf("pos: %f %f %f\n", cam->pos.x, cam->pos.y, cam->pos.z);

	//UpdateShake
	V2 shakeOffset = v2(0, 0);
	if(cam->shakeTimer >= 0) {
		cam->shakeTimer += dt;

		float canVal = cam->shakeTimer / 0.5f;

		float inverseTime = 1.0f - canVal;

		shakeOffset.x = cam->shakeMagnitude*inverseTime*lerp(-1, perlin1d(canVal, 10, 4), 1);
		shakeOffset.y = cam->shakeMagnitude*inverseTime*lerp(-1, perlin1d(canVal, 10, 4), 1);
	...
	cam->pos = cam->hidden_pos;
	cam->pos.xy = v2_plus(cam->pos.xy, shakeOffset);
