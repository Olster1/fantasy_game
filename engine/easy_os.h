...
void easyOS_setupApp(OSAppInfo *result, V2 *resolution, char *resPathFolder) {
    ...
    // easyFont_createSDFFont(concatInArena(globalExeBasePath, "/fonts/VT323-Regular.ttf", &globalPerFrameArena), 0, 255);    
...
static inline void easyOS_processKeyStates(OSAppInfo *appInfo, AppKeyStates *state, V2 resolution, V2 *screenDim, bool *running, bool stretched) {
	...
	/////
	easyMemory_zeroArray(state->gameButtons);
